...
**アラートは3本。そして1本目より2本目が大事です。**

- 本番の**5xx**
- **15分間、ログが1行も来ない**
- DBの**p95が1.5秒**を超えた

2本目を説明します。ヘルスチェックは`/api/health`を叩いて「返ってくるか」を見ますが、**それはFlyの中で動いています**。Fly側でネットワークが死んだり、マシンが起動すらしなかったりすると、**チェックする主体ごと消えるので、失敗の通知も来ません**。落ちていることを検知できるのは「本来あるはずの音が消えたこと」だけです。

> **監視対象の内側で動いている監視は、監視対象と一緒に死にます。**

3本目は、壊れる前に鳴らすためのものです。DBのp95が1.5秒を超えるのは、まだエラーになっていないが、もうすぐなる状態です。5xxが出てから知るのでは遅い。

**そして、まだできていないこと。** 外部からの死活監視（UptimeRobotやBetter Stackのようなもの）は入れていません。「ログが15分来ない」でかなりの部分は拾えますが、**完全に外側から見ている目はまだない**という穴は自覚しています。
