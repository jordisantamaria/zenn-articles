...
そして多くのプラットフォームのデフォルトは**ローリングデプロイ**で、マシンが1台しかないと「古いのを止めてから新しいのを起動する」になります。台数がないと、ローリングは実質ただの再起動です。
...
これは**ブルーグリーンデプロイ（blue-green deployment）**と呼ばれる方式で、たいていのサーバー型プラットフォームに用意されています。Fly.ioなら設定1行です。2台目はデプロイ中しか存在しないので、費用もほとんど増えません。
