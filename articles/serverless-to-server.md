...
→ **Fly.ioのオートスケール。ただし上限2台。**

先に構成をはっきりさせておくと、**普段動いているのは1台だけ**です。2台目は「枠」であって、**停止したまま待機しています**。起き上がるのは、デプロイのときと、同時接続が閾値を超えたときだけ。**常時2台構成ではありません。**

天井を2に決めた理由の1つはコストです。上限なしにすると、ボットの波がそのまま請求額になります。**弾力性より封じ込めを選ぶ**という、技術ではなくリスクの判断です。

ただ、理由はそれだけではありません。**そもそも、台数を増やしても速くならないからです。**

理由はNeon側にあります。今の設定はこうです。

| | 値 |
|---|---|
| コンピュート | **0.25 CU 固定**（最小＝最大） |
| `max_connections` | **112**（うち6はsuperuser予約） |

**アプリを何台に増やしても、この0.25 CUは1ミリも増えません。** ボトルネックがDB側にあるなら、アプリを横に並べるのは「同じ窓口に並ぶ列を増やす」だけです。窓口は1つのままなので。

しかも各インスタンスは**自分のコネクションプールを持ちます**。台数×プールが、増えない106本を取り合う。**キャパシティが上がる前に、コネクションの奪い合いのほうが先に起きます。**

つまり、Fly.ioのマシンが月$3で追加できることには意味がありません。**安いほうを増やしても、詰まっているのは高いほうだからです。** 本当にスケールさせるなら、増やすのはコンテナではなくNeonのプランのほうです。

だから2台目は、**速くするための2台目ではなく、止まらないための2台目**です。ほとんどの時間は眠っていて、それでいい。
