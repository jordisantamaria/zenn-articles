...
正直に書いておきます。**エージェントがやってくれるのはデータと仮説までで、決めるのは自分です。**

証拠もあります。この記事に出てくる`auto_stop_machines = false`という間違いは、**まさにこの流れで書かれたもの**です。「止めたくないんだから`false`」は、もっともらしくて、間違っていた。気づいたのは、後からドキュメントを読み直した人間のほうでした。

数字を集めるのはエージェントのほうが速い。**その数字が何を意味するかを決めるのは、まだこちらの仕事です。**
